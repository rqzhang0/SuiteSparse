...
cs *cs_multiply(const cs *A, const cs *B) {
  ...
  //分配C矩阵的内存,
  // anz是A矩阵非零元素个数，bnz是B矩阵的非零元个数,给C矩阵分配内存为A,B矩阵中非零元个数之和
  //为什么要分配为A,B矩阵非零元个数之和？
  //对于矩阵乘法，C矩阵的矩阵元比A,B矩阵的矩阵元多是非常常见的情况(比如A:9*3;
  // B:3*16 => C:9*16)
  ...
  for (j = 0; j < n; j++) {
    ...
    //对B的列中的每一行进行循环,此处相当于已经判定B的非零元
    for (p = Bp[j]; p < Bp[j + 1]; p++) {
      //此处执行的应该是C_*j=\sum_k A_*k B_kj
      //B_kj已经非零所以无需对其进行判断，只需判定A中的非零元素
      nz = cs_scatter(A, Bi[p], Bx ? Bx[p] : 1, w, x, j + 1, C, nz);
    }
    if (values)
    //将x中存储的数据拷贝到Cx中, Ci在cs_scatter中进行更新
      for (p = Cp[j]; p < nz; p++)
        Cx[p] = x[Ci[p]];
  }
  //最后一列的列指标数据进行更新
